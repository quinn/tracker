...
fn remove_request_hop_by_hop_headers(headers: &mut HeaderMap) {
    for h in HOP_BY_HOP_HEADERS.iter() {
        if headers.contains_key(h) {
            let header_value = headers.get(h).unwrap().to_str().unwrap_or("");

            if header_value == "" || (h == *HEADER_TE && header_value == "trailers") {
                continue;
            }
        }
...
impl<'a> ReverseProxy<'a> {
    ...
    pub fn forward(&self, req: HttpRequest) -> impl Future {
        let mut forward_req = client::ClientRequest::from(&req);
